...
std::vector<TString> calc_polarization(TFile *infile, int run_num, int accum, int beamCut=1){
  ...
  TH1F *hAsymOn  = (TH1F *)infile->Get(Form("%s_beam%i_las1_asym%isubAvg", tree_name.Data(), beamCut, accum));
  ...
  TH1F *hAsymOff = (TH1F *)infile->Get(Form("%s_beam%i_las0_asym%isubAvg", tree_name.Data(), beamCut, accum));
  ...
  Double_t pol = vAsymSub/get_analyzing_power(run_num); Double_t polE = vAsymSubE/get_analyzing_power(run_num);
  ...
  results.push_back(Form("BkSub Asym: %f +/- %f\n", vAsymSub, vAsymSubE));
...
void snapshots_pad(TFile *infile, int run_num, TString output_path, TPad *myPad, bool printfile=false){
  ...
  if(printfile)
    myPad->Print(Form("%s/snapshots.png", output_path.Data()), "png");
}
...
void asym_pad(TFile *infile, int run_num, TString output_path, TPad *myPad, int accum=0, bool printfile=false){
  ...
  TString msmts[3] = {Form("posH%i", accum), Form("negH%i", accum), Form("asym%isubAvg", accum)};
  for(int i = 0;  i < 3; i++){
    myPad->cd(i + 1);
