...
void writeToPDF(int run_num){
  ...
  essential_stats_pad(infile, run_num, output_path, c_ess_stats, true);
  snapshots_pad(infile, run_num, output_path, c_snap, true);
  breakdown_pad(infile, run_num, output_path, c_sums, "triggerwise", "sums", true);
  breakdown_pad(infile, run_num, output_path, c_acc0, "mpswise", "acc0", true);
  acc0_time_pad(infile, run_num, output_path, c_acc0_time, true);
  quartet_pad(infile, run_num, output_path, c_diff0, "diff0");
  quartet_pad(infile, run_num, output_path, c_summ0, "summ0");
  quartet_pad(infile, run_num, output_path, c_asym0, "asym0");
  asym_pad(infile, run_num, output_path, c_pol0, 0, true);
  asym_pad(infile, run_num, output_path, c_pol4, 4, true);
  asym_graph_pad(infile, run_num, output_path, c_pol0_graphs, 0, true);
  asym_graph_pad(infile, run_num, output_path, c_pol4_graphs, 4, true);
}
