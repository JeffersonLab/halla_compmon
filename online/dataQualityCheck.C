...
void epicsPlots(TChain *epicswise, int run_num, TFile *outfile, ofstream *textOut){
  ...
  (*textOut)<<year<<"-"<<translate_month(month)<<"-"<<date_num<<" "<<hour<<":"<<minute<<":"<<second<<"\n";
}
...
void dataQualityCheck(int run_num, int max_evt=1e9){
  ...
  ofstream textOut; textOut.open(textOutName.Data());
  epicsPlots(runChains[epicswise], run_num, fout, &textOut);
  ...
  textOut<<(Int_t)runChains[mpswise]->GetEntries(); textOut.close();
