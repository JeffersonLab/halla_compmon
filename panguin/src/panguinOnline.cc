  ...
  fVerbosity(ver)
{
  ...
  if(printonly) {
    ...
    fPrintOnly=kFALSE;
    CreateGUI(gClient->GetRoot(),1200,800);
  }
}
...
void OnlineGUI::CreateGUI(const TGWindow *p, UInt_t w, UInt_t h) 
{
  ...
  TString path(getenv("COMPMON_PANGUIN"));
  if(!fConfig->IsMonitor()) {
    wile = 
      new TGPictureButton(vframe,gClient->GetPicture(path+"/"+guiDirectory+"/compton.xpm"));
    wile->Connect("Pressed()","OnlineGUI", this,"DoDraw()");
  } else {
    wile = 
      new TGPictureButton(vframe,gClient->GetPicture(path+"/"+guiDirectory+"/panguin.xpm"));
