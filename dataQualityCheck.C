...
void epicsPlots(TChain *epicswise, int run_num, TFile *outfile){
  ...
  TH1F *h_day      = new TH1F("h_day",      "Day of Week",    7,    1,    7); h_day->Fill(translate_day(day));
  TH1F *h_month    = new TH1F("h_month",    "Month of Year", 12,    1,   12); h_month->Fill(translate_month(month));
  TH1F *h_date_num = new TH1F("h_date_num", "Day Number",    31,    1,   31); h_date_num->Fill(date_num);
  TH1F *h_year     = new TH1F("h_year",     "Year",          50, 2001, 2050); h_year->Fill(year);
  TH1F *h_hour     = new TH1F("h_hour",     "Hour",          24,    0,   23); h_hour->Fill(hour);
  TH1F *h_minute   = new TH1F("h_minute",   "Minute",        60,    0,   59); h_minute->Fill(minute);
  TH1F *h_second   = new TH1F("h_second",   "Second",        60,    0,   59); h_second->Fill(second);
...
void quartet_plots(TChain* somethingwise, int run_num, TFile* outfile, int max_event){
  ...
  TString diff0("PosHelAcc0/PosHelNSamples0 - NegHelAcc0/NegHelNSamples0");
  TString summ0("PosHelAcc0/PosHelNSamples0 + NegHelAcc0/NegHelNSamples0");
  TString asym0("(PosHelAcc0/PosHelNSamples0 - NegHelAcc0/NegHelNSamples0)/(PosHelAcc0/PosHelNSamples0 + NegHelAcc0/NegHelNSamples0)");
  TString posH4("PosHelAcc4");
  TString negH4("NegHelAcc4");
  TString diff4("PosHelAcc4 - NegHelAcc4");
  TString summ4("PosHelAcc4 + NegHelAcc4");
  TString asym4("(PosHelAcc4 - NegHelAcc4)/(PosHelAcc4 + NegHelAcc4)");

  TString meas[10] = {posH0, negH0, diff0, summ0, asym0, posH4, negH4, diff4, summ4, asym4};
  TString meas_id[10] = {"PosHelAcc0/NSamples0", "NegHelAcc0/NSamples0", "Diffs0", "Sums0", "Asyms0", "PosHelAcc4", "NegHelAcc4", "Diffs4", "Sums4", "Asyms4"};
  TString meas_small[10] = {"posH0", "negH0", "diff0", "summ0", "asym0", "posH4", "negH4", "diff4", "summ4", "asym4"};
  ...
  for(int i = 0; i < 10; i++){
    ...
    for(int j = 0; j < 2; j++){
      ...
      for(int k = 0; k < 2; k++){
        ...
        TH1F *h3; make_histo(somethingwise, h3, hname3, title3, meas[i], cuts3); write_histo(hname3, outfile);
      }
    }
  }
}
...
void quartet_graphs(TChain* somethingwise, int run_num, TFile* outfile, TString tree_id, int max_event){
  ...
  Int_t n_msmts = 11;
  TString msmts[11] = {"posH0", "negH0", "diff0", "summ0", "asym0", "bcm", "posH4", "negH4", "diff4", "summ4", "asym4"};
  ...
  Float_t posBCM, negBCM;

  somethingwise->SetBranchAddress("PosHelBCM", &posBCM);
  somethingwise->SetBranchAddress("NegHelBCM", &negBCM);
  somethingwise->SetBranchAddress("laserState", &laserState); somethingwise->SetBranchAddress("beamState", &beamState);
  ...
  Double_t ymins[11] = { 1e6,  1e6,  1e6,  1e6,  1e6,  1e6,  1e6,  1e6,  1e6,  1e6,  1e6};
  Double_t ymaxs[11] = {-1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6};
  for(int i = 0; i < somethingwise->GetEntries(); i++){
    somethingwise->GetEntry(i);
    if(firstMPSnumber > max_event){continue;}
    Int_t laserInd = (Int_t)(laserState==0 || laserState==1);
    Int_t beamInd  = (Int_t)(beamState==1);
    Double_t msmt[11] = {posAcc/posSamp, negAcc/negSamp, posAcc/posSamp - negAcc/negSamp, posAcc/posSamp + negAcc/negSamp, 
                        (posAcc/posSamp - negAcc/negSamp)/(posAcc/posSamp + negAcc/negSamp), (posBCM + negBCM)/2.0, posAcc4/posSamp4, negAcc4/negSamp4, 
                        posAcc4/posSamp4 - negAcc4/negSamp4, posAcc4/posSamp4 + negAcc4/negSamp4,
                        (posAcc4/posSamp4 - negAcc4/negSamp4)/(posAcc4/posSamp4 + negAcc4/negSamp4)};
