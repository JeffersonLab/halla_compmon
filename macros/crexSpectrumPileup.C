...
// *********************************************
void crexSpectrumPileup(Int_t runNum){
  TString legTitle = Form("Run %i (Reg. Subtraction)", runNum);
  ...
  TFile *file = TFile::Open(Form("%s/compmon_%i.root", getenv("COMP_ROOTFILES"), runNum));
  TTree *trig = (TTree *)file->Get("triggerwise");
  Int_t nBins = 500;
  Float_t clock = getClockCutoff(trig, runNum);
  ...
  TH1F *hSum = new TH1F(Form("hSum_run%i", runNum), Form("Run %i Spectrum", runNum), nBins, 0, 75e3);
  trig->Project(Form("hSum_run%i", runNum), "sum", cuts.Data(), "goff");

  Float_t ce = findComptonEdge(runNum, hSum, nBins);
  Float_t fac = getSubtractionFactor(trig, runNum, ce, clock, 1.0);

  nBins = 1000;
  vector<TH1F *> hists = getHistograms(nBins);
  hists = fillHistogram(trig, hists, runNum, ce, clock);
  hists[0]->SetLineColor(kBlue);
  hists[2]->SetLineColor(kBlue);
  hists[0]->Add(hists[2], -fac);

  Float_t scale = getNormFactor(hists[0], nBins);
  hists[0]->Scale(1.04/scale);
  hists[1]->Scale(1.04/scale);
  hists[2]->Scale(fac*1.04/scale);
    
  leg->AddEntry(hists[0], legTitle);
  legSub->AddEntry(hists[2], legTitle);
  ...
  Int_t matchBinNum = 1;
  while(hists[0]->GetBinCenter(matchBinNum) < 1.5 && matchBinNum <= nBins){
    matchBinNum++;
  }
  fillSimHist_6mm(hs, leg, kBlack, hists[0]->GetBinContent(matchBinNum));
