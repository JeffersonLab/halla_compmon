...
void makeRunPedestalPlots(Int_t runNum, vector<vector<Int_t>> cycles){
  ...
  int randoms = 0;
  TH1F *hON = new TH1F(Form("hON_run%i", runNum), Form("Run %i Lser ON", runNum), 100, 3780, 3798);
  gStyle->SetOptStat();
  gStyle->SetStatFormat("6.6g");
  hON->SetLineColor(kGreen + 2); hON->SetStats(0); 
  hON->GetXaxis()->SetTitle("Pedestal Mean [RAU]");
  TH1F *hOFF = new TH1F(Form("hOFF_run%i", runNum), Form("Run %i Laesr OFF", runNum), 100, 3780, 3798);
  ...
  for(Int_t i = 0; i < snapshots->GetEntries(); i++){
    ...
    sum /= 40.0;
    if((laserState == 0 || laserState == 1) && randomTime == 1) hON->Fill(sum);
    else if((laserState == 2 || laserState == 3) && randomTime == 1) hOFF->Fill(sum);
  }
